...
@interface XKPhotoView () <UIScrollViewDelegate>

@property (weak, nonatomic) UIImageView *imageView;

@end

@implementation XKPhotoView {
...
- (void)displayImage:(NSURL *)aImageUrl
{
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 95, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height - 320)];
    imageView.contentMode = UIViewContentModeScaleAspectFit;
    imageView.userInteractionEnabled = YES;
//    [imageView sd_setImageWithURL:aImageUrl];
//    [imageView sd_setImageWithURL:aImageUrl placeholderImage:[UIImage imageNamed:@"jzfpzanwushuju"]];
...
//        CGFloat newScale = MIN(self.zoomScale * kZoomStep, self.maximumZoomScale);
        [self updateZoomScaleWithGesture:gestureRecognizer newScale:self.maximumZoomScale];
    }
...
- (void)handleScrollViewDoubleTap:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (!CGPointEqualToPoint(center, CGPointZero)) {
        CGFloat newScale = MIN([self zoomScale], self.maximumZoomScale);
